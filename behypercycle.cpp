 ...
 */
gsl_matrix *solve_lin_prog(double constr, gsl_matrix *A, gsl_vector *x, int sizeA, float f)
{
	/* Find the vector v */
...
int main(int *argc, char **argv)
{
    ...
    *     ODE decision step (at which iterations of evolution we will solve an ODE) 
   */
    ...
    /*time step (grid for solving ODE)*/
    cout << "Enter the time step (grid for solving ODE) "; cin >> h; cout << endl;
    /*ODU decision step (at which iterations of evolution we will solve an ODE) */
    cout << "Enter ODE decision step (at which iterations of evolution we will solve an ODE)  "; cin >> solve_step; cout << endl;    
    ...
    ifstream fin_B_constr("constr.txt");// opened the file to read the restrictions on the elements bij
    ...
    for(int i = 0; i <= count_iter; i++)
    {
        ...
        */
        B = solve_lin_prog(constr, A, x, sizeA, gsl_vector_get(fitness_vec, i));
